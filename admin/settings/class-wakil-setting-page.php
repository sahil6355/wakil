...
if ( ! class_exists( 'wakil_Settings_Page', false ) ) :

	/**
	 * wakil_Settings_Page.
	 */
	abstract class wakil_Settings_Page {
		...
		public function __construct() {
			add_filter( 'wakil_settings_tabs_array', array( $this, 'add_settings_page' ), 20 );

			add_action( 'wakil_sections_' . $this->id, array( $this, 'output_sections' ) );

			add_action( 'wakil_settings_' . $this->id, array( $this, 'output' ) );

			add_action( 'wakil_settings_save_' . $this->id, array( $this, 'save' ) );
		 ...
		 *
		 * External settings classes (registered via 'wakil_get_settings_pages' filter)
		...
		final public function get_settings_for_section( $section_id ) {
			...
			return apply_filters( 'wakil_get_settings_' . $this->id, $settings, $section_id );
		}
		 ...
		 *
		 * When overriding, note that the 'wakil_get_settings_' filter must NOT be triggered,
		...
		public function get_sections() {
			...
			return apply_filters( 'wakil_get_sections_' . $this->id, $sections );
		}
		...
		public function output_sections() {
			...
			foreach ( $sections as $id => $label ) {
				$url       = admin_url( 'admin.php?page=main-wakil&tab=' . esc_attr($this->id, 'wakil') . '&section=' . sanitize_title( $id ) );
				$class     = ( $current_section === $id ? 'current' : '' );
				$separator = ( end( $array_keys ) === $id ? '' : '|' );
				$text      = esc_html__( $label , 'wakil');
				echo "<li><a href='".esc_url($url)."' class='".esc_attr($class)."'>".esc_html__($text, 'wakil')."</a> ".esc_html__($separator, 'wakil')." </li>"; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		...
		public function output() {
			...
			Wakil_Admin_Settings::output_fields( $settings );
		}

		/**
		 * Save settings and trigger the 'wakil_update_options_'.id action.
		 */
		...
		protected function save_settings_for_current_section() {
			...
			// for compatibility with derived classes overriding "get_settings".
			$settings = $this->get_settings( $current_section );
			Wakil_Admin_Settings::save_fields( $settings );
		}

		/**
		 * Trigger the 'wakil_update_options_'.id action.
		 *
		...
		protected function do_update_options_action( $section_id = null ) {
			...
			if ( $section_id ) {
				do_action( 'wakil_update_options_' . $this->id . '_' . $section_id );
			}
		}
	}
